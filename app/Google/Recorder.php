...
class Recorder
{
    ...
    protected string $command = 'ffmpeg -i {url} -b {bitrate}k -vcodec copy -r {fps} -y {filepath}';
    ...
    public function __construct(protected LivestreamLoop $loop)
    {
        ...
        $this->attributes = new Attributes([
            ...
            'extension'  => 'avi',
            'split_time' => 60 * 60,
            'device'     => $loop->getDevice()->toArray(),
        ]);
        $this->loop->onStart(function(){
            $this->start();
        });
        $this->loop->onStop(function(){
            $this->stop();
        });
    }
